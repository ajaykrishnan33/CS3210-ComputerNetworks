	...
	public void run() {
		...
		while(true){
			...
			if(GlobalVariables.last_sent==GlobalVariables.window_end || GlobalVariables.unacked_packet_count>=GlobalVariables.WINDOW_SIZE){
				// System.out.println("unacked_packet_count: "+GlobalVariables.unacked_packet_count);
				GlobalVariables.lock.unlock();
				continue;
			}
			...
			try{
				...
				// System.out.println("unacked_packet_count1: "+this.unacked_packet_count);
	...
	public void cancelAllPreviousTimersAndPackets(long currTime){
		...
		for(Packet p:GlobalVariables.packetQ){
			if(p.timer!=null){
				...
				// System.out.println("unacked_packet_count2: "+this.dst.unacked_packet_count);
				...
				if(i==last_acked){
					this.computeRTT();
					// System.out.println("Avg. RTT: "+ GlobalVariables.avgRTT);
					break;
				}
				i = (i + 1)%base;
			}
