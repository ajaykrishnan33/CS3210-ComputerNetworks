...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    childfd = parentfd;
    // recvfrom(parentfd, (struct sockaddr *) &clientaddr, &clientlen);
    ...
    if (hostaddrp == NULL)
      error("ERROR on inet_ntoa\n");
    printf("server established connection with (%s:%d)\n", 
    ...
    n = sendto(childfd,buf, BUFSIZE, 0,(struct sockaddr *)&clientaddr, clientlen);
